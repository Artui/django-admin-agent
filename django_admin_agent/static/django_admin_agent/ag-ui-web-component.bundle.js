...
`,1)[0],A;if(x=v,this.options.pedantic?(x=x.replace(this.rules.other.listReplaceNesting,"  "),A=x):A=x.replace(this.rules.other.tabCharGlobal,"    "),_.test(x)||S.test(x)||L.test(x)||F.test(x)||b.test(x)||w.test(x))break;if(A.search(this.rules.other.nonSpaceChar)>=p||!x.trim())d+=`
