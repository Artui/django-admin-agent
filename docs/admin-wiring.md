...
| `user_name` | `request.user.get_short_name()`, else `get_username()` (or `None`) | The `user-name` attribute: the name the greeting on an empty `placement="page"` conversation uses (*Hello, {name}*). The same name the admin's own header welcomes the user by. Presentation only: the component never sends it and scopes nothing by it. Other placements show no greeting. `None` (attribute omitted, *Hello there*) for an anonymous or absent user. |
